...
defmodule Mix.Tasks.PhxAdmin.InstallTest do
  use ExUnit.Case
  import MixHelper
  ...
  defp assert_brunch_assets(mode \\ :assert ) do
    ...
    dest_path = ~w(web static assets images phx_admin)
  ...
  defp assert_no_brunch_assets(mode \\ :assert) do
    ...
    dest_path = ~w(priv static images phx_admin)
    Enum.each @image_files, fn file ->
      assert_or_refute_file mode, Path.join(dest_path ++ [file])
    end
    dest_path = dest_path ++ ["datepicker"]
  ...
  def assert_dashboard do
    ...
    assert_file path, fn file ->
      assert file =~ "defmodule PhxAdmin.PhxAdmin.Dashboard do"
      assert file =~ "Welcome to PhxAdmin."
    end
  end
