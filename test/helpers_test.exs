defmodule PhxAdmin.HelpersTest do
  use ExUnit.Case
  alias PhxAdmin.Helpers
  ...
  test "build_field Actions" do
    ...
    defn = %TestExAdmin.PhxAdmin.Simple{}
    ...
    res = Helpers.build_field(resource, conn, {"Actions", %{fun: fn(res) ->
      PhxAdmin.Index.build_index_links(conn, res, [:show, :edit, :delete])
    end}},
  ...
  test "build_field with complex map data" do
    ...
    res = Helpers.build_field(resource, %{}, {:stats, %{}}, fn(contents, _field_name) ->
        PhxAdmin.Render.to_string(contents)
      end)
    assert res == ~s(list: [{}])
  end
