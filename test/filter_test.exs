defmodule PhxAdmin.FilterTest do
  use ExUnit.Case, async: true
  alias PhxAdmin.Filter

  ############
  # filters

  test "filters" do
    defn = %TestExAdmin.PhxAdmin.User{}
    assert Filter.fields(defn) == [name: :string, email: :string]
  end
  test "filters except" do
    defn = %TestExAdmin.PhxAdmin.User{index_filters: [[except: [:name, :email]]]}
  ...
  test "filters only" do
    defn = %TestExAdmin.PhxAdmin.User{index_filters: [[only: [:name, :active]]]}
    assert Filter.fields(defn) == [name: :string, active: :boolean]
  end
  test "filters only field_label" do
    defn = %TestExAdmin.PhxAdmin.User{index_filters: [[only: [:name, :email], labels: [email: "EMail Address"]]]}
    assert Filter.fields(defn) == [name: :string, email: :string]
  end
  test "filters except field_label" do
    defn = %TestExAdmin.PhxAdmin.User{index_filters: [[except: [:active], labels: [email: "EMail Address"]]]}
    assert Filter.fields(defn) == [name: :string, email: :string]
  end
  ...
  test "filters except several fields" do
    defn = %TestExAdmin.PhxAdmin.Noprimary{index_filters: [[except: [:inserted_at, :updated_at]]]}
  ...
  test "filter_label label" do
    defn = %TestExAdmin.PhxAdmin.User{index_filters: [[labels: [email: "EMail Address", name: "Full Name"]]]}
