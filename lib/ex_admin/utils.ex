defmodule PhxAdmin.Utils do
  ...
  if @module do
    ...
    Logger.warn """
    PhxAdmin requires recompiling after adding :phx_admin configuration in your config/config.exs file.
    After running 'mix admin.install' and updating your config.exs file, please
    run 'touch deps/phx_admin/mix.exs && mix deps.compile phx_admin'.
    """
    @doc false
    def endpoint, do: Module.concat([Application.get_env(:phx_admin, :module), "Endpoint"])
    @doc false
    def router, do: Module.concat([Application.get_env(:phx_admin, :module), "Router", "Helpers"])
  end
  ...
  ## Examples
      ...
      iex> PhxAdmin.Utils.titleize "MyModel"
      "My Model"
  ...
  def admin_resource_path(resource_model, method, args) when is_atom(resource_model) do
    resource_name = resource_model |> PhxAdmin.Utils.base_name |> Inflex.underscore |> Inflex.pluralize
  ...
  def admin_resource_path(resource, method, args) when is_map(resource) do
    ...
    id = PhxAdmin.Schema.get_id(resource)
    case id do
      nil ->
  ...
  def admin_association_path(resource, assoc_name, method \\ nil, args \\ []) do
    resource_model = resource.__struct__
    resource_id = PhxAdmin.Schema.get_id(resource)
  ...
  def displayable_name_plural(conn) do
    PhxAdmin.Utils.get_resource_label(conn) |> Inflex.pluralize
  end
  def displayable_name_singular(conn) do
    PhxAdmin.Utils.get_resource_label(conn) |> Inflex.singularize
  end

  @doc false
  def authorized_action?(conn, action, resource_model) when is_atom(resource_model) do
    # fun = Application.get_env(:phx_admin, :authorize)
  ...
  def authorized_action?(conn, action, resource) do
    PhxAdmin.Authorization.authorize_action(resource, conn, action)
  end
  #def authorized_action?(conn, action) do
  #  PhxAdmin.Authorization.authorize_action(conn.assigns[:defn], conn, action)
  #end
