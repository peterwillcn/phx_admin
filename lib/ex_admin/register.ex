defmodule PhxAdmin.Register do
  ...
  Register the Survey.Answer model with all defaults.
      ...
      defmodule Survey.PhxAdmin.Answer do
  ...
  * `changesets` - Defines custom changeset functions
  ...
  import PhxAdmin.DslUtils

  defmacro __using__(_) do
    quote do
      use PhxAdmin.Index, except: [actions: 1]
      use PhxAdmin.Show
      use PhxAdmin.Form, except: [actions: 1]
      use PhxAdmin.CSV
  ...
  Once registered, PhxAdmin adds the resource to the administration
  ...
  * A CSV export link on the index page
  ...
  PhxAdmin will render an association using the following algorithm in the following order:
  ...
  * Use the 2nd field in the Model's schema
  ...
  defmacro register_resource(mod, [do: block]) do
    quote location: :keep do
      ...
      @name_column Module.get_attribute(__MODULE__, :name_column) || apply(PhxAdmin.Helpers, :get_name_field, [module])
      ...
      query_opts = case Module.get_attribute(__MODULE__, :query) do
        nil ->
          ...
          |> Enum.map(&(PhxAdmin.Register.build_query_association module, &1))
      ...
      |> PhxAdmin.Helpers.group_reduce_by_reverse

      action_labels = PhxAdmin.Register.get_action_labels(Module.get_attribute(__MODULE__, :actions))
      actions =
        PhxAdmin.Register.get_action_items(Module.get_attribute(__MODULE__, :actions), @all_options)
        |> PhxAdmin.Register.custom_action_actions(Module.get_attribute(__MODULE__, :member_actions), module, :member_actions)
        |> PhxAdmin.Register.custom_action_actions(Module.get_attribute(__MODULE__, :collection_actions), module, :collection_actions)

      defstruct controller: @controller,
                controller_methods: Module.get_attribute(__MODULE__, :controller_methods),
                title_actions: &PhxAdmin.default_resource_title_actions/2,
                type: :resource,
      ...
      def run_query(repo, defn, action, id \\ nil) do
        ...
        |> PhxAdmin.Query.run_query(repo, defn, action, id, @query)
      end
      def run_query_counts(repo, defn, action, id \\ nil) do
        %__MODULE__{}
        |> Map.get(:resource_model)
        |> PhxAdmin.Query.run_query_counts(repo, defn, action, id, @query)
      end
      ...
      fun = quote do
        ...
        if type == :member_actions do
          fn id ->
            ...
            url = PhxAdmin.Utils.admin_resource_path(resource, :member, [name])
            PhxAdmin.ViewHelpers.action_item_link human_name, href: url, "data-method": :put
          end
        else
          fn id ->
            resource = module
            url = PhxAdmin.Utils.admin_resource_path(resource, :collection, [name])
            PhxAdmin.ViewHelpers.action_item_link human_name, href: url
          end
        end
      end
  ...
  * `plug` - Add a plug to the controller
  ...
  defmacro register_page(name, [do: block]) do
    quote location: :keep do
      import PhxAdmin.Register, except: [column: 1]
      use PhxAdmin.Page
      ...
      defstruct controller: Module.concat(Application.get_env(:phx_admin, :project), AdminController),
                controller_methods: Module.get_attribute(__MODULE__, :controller_methods),
                type: :page,
                page_name: page_name,
                title_actions: &PhxAdmin.default_page_title_actions/2,
  ...
  * `:except` - Filters out actions in the except atom or list.
  ...
  ## Examples
      ...
      sidebar "PhxAdmin Demo", only: [:index, :show] do
  ...
  * `collection` - Add the collection for a belongs_to association
  ...
  defmacro column(name, opts \\ [], fun \\ nil) do
    quote do
      opts = PhxAdmin.DslUtils.fun_to_opts unquote(opts), unquote(fun)
      var!(columns, PhxAdmin.Show) = [{unquote(name), (opts)} | var!(columns, PhxAdmin.Show)]
    end
  end
  ...
  defmacro row(name, opts \\ [], fun \\ nil) do
    quote do
      opts = PhxAdmin.DslUtils.fun_to_opts unquote(opts), unquote(fun)
      var!(rows, PhxAdmin.Show) = [{unquote(name), opts} | var!(rows, PhxAdmin.Show)]
    end
  end
  ...
  * `action_items` macro will not remove any custom actions defined by the `action_item` macro.
  ...
  a backup and restore page.
      ...
      def restore_action(conn, params) do
        ...
        |> Controller.redirect(to: PhxAdmin.Utils.admin_resource_path(conn, :index))
      end
  ...
  * an optional label: "Button Label" (shown above)
  ...
  defp check_preload(field, key) do
    if Application.get_env :phx_admin, key, true do
      field
    else
      nil
    end
