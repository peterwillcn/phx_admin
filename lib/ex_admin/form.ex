defmodule PhxAdmin.Form do
  @moduledoc """
  Override the default new and edit form pages for an PhxAdmin resource.

  By default, PhxAdmin renders the form page without any additional
  ...
  PhxAdmin supports in-line creation of a has_many, through: (many-to-many)
  ...
  use PhxAdmin.Adminlog

  import PhxAdmin.Utils
  import PhxAdmin.Helpers
  import PhxAdmin.DslUtils
  import PhxAdmin.Form.Fields
  import PhxAdmin.ViewHelpers, only: [escape_javascript: 1]
  require IEx
  import PhxAdmin.Theme.Helpers
  ...
  defmacro form(resource, [do: contents]) do
    quote location: :keep, bind_quoted: [resource: escape(resource), contents: escape(contents)] do
      import PhxAdmin.Index, only: [index: 1]
      def form_view(var!(conn), unquote(resource) = var!(resource), var!(params) = params) do
        import PhxAdmin.Register, except: [actions: 1]

        var!(input_blocks, PhxAdmin.Form) = []
        var!(script_block, PhxAdmin.Form) = nil
        unquote(contents)
        items = var!(input_blocks, PhxAdmin.Form) |> Enum.reverse
        script_block = var!(script_block, PhxAdmin.Form)
        Module.concat(var!(conn).assigns.theme, Form).build_form(
          var!(conn), var!(resource), items, var!(params), script_block, PhxAdmin.Form.global_script)
      end
      ...
      def get_blocks(var!(conn), unquote(resource) = var!(resource), var!(params) = _params) do
        ...
        var!(script_block, PhxAdmin.Form) = nil
        unquote(contents)
        var!(input_blocks, PhxAdmin.Form) |> Enum.reverse
      end
      ...
      def ajax_view(conn, params, resource, resources, block) do
        ...
        view = markup safe: true do
          PhxAdmin.Form.Fields.ajax_input_collection(resource, resources, model_name, field_name, params[:id1], params[:nested2], block, conn.params)
        end
  ...
  defmacro inputs(name, opts, do: block) do
    quote location: :keep do
      import Xain, except: [input: 1]
      # inputs_save = var!(inputs, PhxAdmin.Form)
      var!(inputs, PhxAdmin.Form) = []
      unquote(block)
      items = var!(inputs, PhxAdmin.Form) |> Enum.reverse
      # var!(inputs, PhxAdmin.Form) = inputs_save
      input_block = %{type: :inputs, name: unquote(name), inputs: items, opts: unquote(opts)}
      var!(input_blocks, PhxAdmin.Form) = [input_block | var!(input_blocks, PhxAdmin.Form)]
    end
  ...
  defmacro inputs(name, opts) do
    quote location: :keep do
      ...
      var!(inputs, PhxAdmin.Form) = [item | var!(inputs, PhxAdmin.Form)]
    end
  end
  ...
  defmacro actions(do: block) do
    quote do
      var!(inputs, PhxAdmin.Form) = []
      var!(items, PhxAdmin.Form) = []
      unquote(block)
      items = var!(items, PhxAdmin.Form) |> Enum.reverse
  ...
  defmacro content(items, opts \\ quote(do: [])) do
    quote do
      ...
      var!(items, PhxAdmin.Form) = [item | var!(items, PhxAdmin.Form)]
    end
  end
  ...
  defmacro javascript(do: block) do
    quote do
      var!(script_block, PhxAdmin.Form) = unquote(block)
    end
  end
      ...
      field ->
        case resource.__struct__.__schema__(:association, field) do
          %Ecto.Association.BelongsTo{cardinality: :one, queryable: assoc} ->
            collection = Application.get_env(:phx_admin, :repo).all assoc
  ...
  defp field_type(resource, field_name) do
    field_type_matching = Application.get_env(:phx_admin, :field_type_matching) || %{}
  ...
  def wrap_item_type(type, label, ext_name, contents, error, required) do
    PhxAdmin.theme.wrap_item_type(type, label, ext_name, contents, error, required)
  end
      ...
      {contents, html} = theme_module(conn, Form).build_inputs_has_many field_name,
        human_label, fn ->
          ...
          |> Enum.map(fn({res, inx}) ->
            ...
            res_id = PhxAdmin.Schema.get_id(res)
            markup do
              html
        ...
        {assoc_model, _} = assoc_model_tuple = PhxAdmin.Repo.get_assoc_model(resource, field_name)
        ...
        selected = cond do
          errors != nil ->
            ...
            ids = Map.get(request_params, model_name, %{}) |>
                  Map.get(name_ids, []) |>
                  PhxAdmin.EctoFormMappers.checkboxes_to_ids
  ...
  def build_item(conn, %{type: :inputs, name: name, opts: %{as: type}} = options,
    ...
    mod = name
          ...
          |> Inflex.singularize
          |> String.to_atom
    module = Application.get_env(:phx_admin, :module)
              |> Module.concat(mod)
  ...
  def build_control(type, resource, opts, model_name, field_name, ext_name) do
    ...
    value = PhxAdmin.Render.to_string(value)
  ...
  def default_form_view(conn, resource, params) do
    case conn.assigns.defn do
      ...
      %{__struct__: _} = defn ->
        ...
        Module.concat(var!(conn).assigns.theme, Form).build_form(
          conn, resource, items, params, false, PhxAdmin.Form.global_script)
    end
  end
