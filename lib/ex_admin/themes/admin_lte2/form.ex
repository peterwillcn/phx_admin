defmodule PhxAdmin.Theme.AdminLte2.Form do
  @moduledoc false
  import Kernel, except: [div: 2]
  import Xain
  import PhxAdmin.Utils
  import PhxAdmin.Form
  require Integer
  ...
  def build_actions_block(conn, _model_name, mode) do
    display_name = PhxAdmin.Utils.displayable_name_singular conn
  ...
  def build_form_error(error) do
    label ".control-label" do
      ...
      text " #{PhxAdmin.Form.error_messages(error)}"
    end
  end
  ...
  # TODO: Refactor some of this back into PhxAdmin.Form
    ...
    div ".box.has_many_fields" do
      ...
      div ".box-body" do
        for {field, type} <- schema do
          ...
          PhxAdmin.Form.build_input(conn, type, field, field_name, res, model_name, error, inx)
        end
        div ".form-group" do
          div ".col-sm-2" do
