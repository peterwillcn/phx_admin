...
defmodule PhxAdmin.Theme.AdminLte2.Filter do
  @moduledoc false
  require Logger
  require Ecto.Query
  import PhxAdmin.Utils
  import PhxAdmin.Gettext
  import PhxAdmin.Filter
  ...
  def build_field({name, %Ecto.Association.BelongsTo{related: assoc, owner_key: owner_key}}, q, defn) do
    ...
    name_field = PhxAdmin.Helpers.get_name_field(assoc)
    ...
    div ".form-group" do
      ...
      select "##{id}.form-control", [name: "q[#{owner_key}_eq]"] do
        option "Any", value: ""
        for r <- resources do
          id = PhxAdmin.Schema.get_id(r)
          name = PhxAdmin.Helpers.display_name(r)
  ...
  def build_field({name, type}, _q, _) do
    Logger.debug "PhxAdmin.Filter: unknown type: #{inspect type} for field: #{inspect name}"
    nil
  end
