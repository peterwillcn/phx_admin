Code.ensure_compiled(PhxAdmin.Utils)
defmodule PhxAdmin.Helpers do
  @moduledoc false
  require Logger
  require Integer
  use Xain
  ...
  def build_link(contents, conn, _, resource, field_name) do
    case Map.get(resource, field_name) do
      nil -> contents
      res when is_map(res) ->
        if PhxAdmin.Utils.authorized_action? conn, :index, res.__struct__ do
  ...
  def model_name(resource) when is_atom(resource) do
    resource |> PhxAdmin.Utils.base_name |> Inflex.underscore()
  end
  ...
  defp build_content_link(link?, conn, resource, contents) do
    if link? && PhxAdmin.Utils.authorized_action?(conn, :show, resource) do
  ...
  defp format_contents(%{} = contents) do
    Enum.reduce(contents, [], fn {k,v}, acc ->
      value = PhxAdmin.Render.to_string(v)
      ["#{k}: #{value}" | acc]
    end)
    |> Enum.reverse
    |> Enum.join(", ")
        ...
        cond do
          ...
          has_function?(struct_name, field, 1) ->
            try_function struct_name, resource, field, fn(_error) ->
              raise PhxAdmin.RuntimeError,
                message: "Could not call resource function #{:field} on #{struct_name}"
            end
          function_exported?(PhxAdmin.get_registered(resource.__struct__).__struct__, :display_name, 1) ->
            apply(PhxAdmin.get_registered(resource.__struct__).__struct__, :display_name, [resource])
          ...
          true ->
            case resource.__struct__.__schema__(:fields) do
              ...
              [id | _] ->
                Map.get resource, id
              _ ->
                raise PhxAdmin.RuntimeError, message: "Could not find field #{inspect field} in #{inspect resource}"
            end
        end
      _ ->
        raise PhxAdmin.RuntimeError, message: "Resource must be a struct"
    end
  end
  ...
  def display_name(resource) do
    defn = PhxAdmin.get_registered(resource.__struct__)
    cond do
      is_nil(defn) ->
