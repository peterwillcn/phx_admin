defmodule PhxAdmin.CSV do
  @moduledoc """
  PhxAdmin provides a CSV export link on the index page of each resource.
  ...
  The following resource file will export the contact list as shown below:
      ...
      defmodule Example.PhxAdmin.Contact do
  ...
  * `column` - Define a column in the exported CSV file
  ...
  defmacro csv(block_or_opts, block) do
    ...
    quote location: :keep do
      import PhxAdmin.Register, except: [column: 1, column: 2]
      import unquote(__MODULE__)

      def build_csv(resources) do
        var!(columns, PhxAdmin.CSV) = []
        unquote(block)
        case var!(columns, PhxAdmin.CSV) do
          [] ->
            unquote(block)
            |> PhxAdmin.CSV.build_csv(resources, unquote(opts))
          schema ->
            schema
            |> Enum.reverse
            |> PhxAdmin.CSV.build_csv(resources, unquote(opts))
  ...
  defmacro column(field, fun \\ nil) do
    quote do
      ...
      var!(columns, PhxAdmin.CSV) = [entry | var!(columns, PhxAdmin.CSV)]
    end
  end
  ...
  defp column_name(field, true), do: PhxAdmin.Utils.humanize(field)
  ...
  def build_row(acc, resource, schema) do
    row = Enum.reduce(schema, [], fn
      %{field: name, fun: nil}, acc ->
        [(Map.get(resource, name) |> PhxAdmin.Render.to_string) | acc]
      %{field: _name, fun: fun}, acc ->
        [(fun.(resource) |> PhxAdmin.Render.to_string) | acc]
    end)
    |> Enum.reverse
    [row | acc]
  end
