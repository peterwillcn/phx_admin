defmodule PhxAdmin.Show do
  @moduledoc """
  Override the default show page for an PhxAdmin resource.

  By default, PhxAdmin renders the show page without any additional
  ...
  import PhxAdmin.Repo, only: [repo: 0]
  ...
  * `all: true` - include fields :id, :inserted_at, :updated_at which are excluded by default.
  ...
  defmacro attributes_table(name, do: block) do
    quote location: :keep do
      var!(rows, PhxAdmin.Show) = []
      unquote(block)
      rows = var!(rows, PhxAdmin.Show) |> Enum.reverse
      schema = %{name: unquote(name), rows: rows}
      PhxAdmin.Table.attributes_table var!(conn), var!(resource), schema
    end
  end

  defmacro attributes_table(_, opts) do
    quote location: :keep do
      PhxAdmin.Show.default_attributes_table(var!(conn), var!(resource), unquote(opts))
    end
  end
  ...
  defmacro attributes_table do
    quote location: :keep do
      PhxAdmin.Show.default_attributes_table(var!(conn), var!(resource))
    end
  end
  ...
  defmacro attributes_table_for(resource, do: block) do
    quote location: :keep do
      ...
      schema = %{rows: rows}
      PhxAdmin.Table.attributes_table_for var!(conn), resource, schema
    end
  end
  ...
  * `contents` - Add HTML to a panel
  ...
  defmacro panel(name \\ "", opts \\ [], do: block) do
    quote do
      var!(elements, PhxAdmin.Show) = []
      unquote(block)
      PhxAdmin.Table.panel(
        var!(conn),
        [ {:name, unquote(name)}, {:opts, unquote(opts)} | var!(elements, PhxAdmin.Show) ], unquote(opts)
      )
    end
  end
  ...
  defmacro table_for(resources, opts, do: block) do
    ...
    quote do
      opts = unquote(opts) |> PhxAdmin.Show.prepare_sortable_opts

      var!(columns, PhxAdmin.Show) = []
      unquote(block)
      columns = var!(columns, PhxAdmin.Show) |> Enum.reverse

      var!(elements, PhxAdmin.Show) = var!(elements, PhxAdmin.Show) ++ [{
        :table_for,
  ...
  def prepare_sortable_opts(opts) do
    case opts[:sortable] do
      [resource: resource, assoc_name: assoc_name] ->
        path = PhxAdmin.Utils.admin_association_path(resource, assoc_name, :update_positions)
        [
  ...
  *Note:* If you have custom keys in intersection table, please use association_filler/2 to specify them explicit.
  ...
  defmacro association_filler(resource, assoc_name, opts) do
    quote bind_quoted: [resource: resource, assoc_name: assoc_name, opts: opts] do
      opts = PhxAdmin.Schema.get_intersection_keys(resource, assoc_name)
  ...
  * `autocomplete` - preload all possible associations if `false` and use autocomplete if `true`
  ...
  defmacro association_filler(resource, opts) do
    quote bind_quoted: [resource: resource, opts: opts] do
      ...
      PhxAdmin.Show.build_association_filler_form(resource, opts[:autocomplete], opts)
    end
  end

  @doc false
  def build_association_filler_form(resource, true = _autocomplete, opts) do
    path = PhxAdmin.Utils.admin_association_path(resource, opts[:assoc_name], :add)
    markup do
      ...
      associations_path = PhxAdmin.Utils.admin_association_path(resource, opts[:assoc_name])
      script type: "text/javascript" do
        text """
        $(document).ready(function() {
          PhxAdmin.association_filler_opts.ajax.url = "#{associations_path}";
          $(".association_filler").select2(PhxAdmin.association_filler_opts);
        });
        """
      end
    end
  ...
  def build_association_filler_form(resource, _autocomplete, opts) do
    ...
    assoc_defn = PhxAdmin.get_registered_by_association(resource, assoc_name)
    path = PhxAdmin.Utils.admin_association_path(resource, opts[:assoc_name], :add)
    ...
    Xain.form class: "association_filler_form", name: "select_#{opts[:assoc_name]}", method: "post", action: path do
      ...
      Xain.select class: "select2", multiple: "multiple", name: "selected_ids[]" do
        PhxAdmin.Model.potential_associations_query(resource, assoc_defn.__struct__, assoc_name)
        |> repo().all
        |> Enum.each(fn(opt) ->
          option PhxAdmin.Helpers.display_name(opt), value: PhxAdmin.Schema.get_id(opt)
        end)
      end
      Xain.input value: "Save", type: "submit", class: "btn btn-primary", style: "margin-left: 1em;"
    end
  ...
  def default_attributes_table(conn, resource, opts \\ []) do
    case conn.assigns.defn do
      ...
      %{__struct__: _} = defn ->
        ...
        PhxAdmin.Table.attributes_table conn, resource, %{rows: columns}
    end
  end
