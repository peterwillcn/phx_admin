defmodule PhxAdmin do
  @moduledoc """
  PhxAdmin is a an auto administration tool for the PhoenixFramework,
  ...
  PhxAdmin adds a menu item for the model in the admin interface, along
  ...
  * `controller` - Customer the controller
  ...
  By default, PhxAdmin used Poison.encode! to encode `Map` type. To change the
  ...
  * Add a controller `before_filter` in your admin resource file.
  ...
  desired type. Here is an example from the PhxAdmin code for the `Ecto.Date` type:
  ...
  to the `<head>` section of PhxAdmin's layout file.
  ...
  * `admin_layout.html` is a template in `web/templates/admin` directory
  ...
  Changing the content of the footer can be done through PhxAdmin's configuration
  options.
  ...
  manually testing an project. PhxAdmin supports this feature through configuration
  and a plug.
  ...
  require Logger
  use Xain
  alias PhxAdmin.Utils
  import PhxAdmin.Utils, only: [titleize: 1, humanize: 1, admin_resource_path: 2]
  require PhxAdmin.Register

  Code.ensure_compiled PhxAdmin.Register
  ...
  case Application.get_env :xain, :after_callback do
    ...
    {_, _} -> nil
    _ ->
      raise PhxAdmin.CompileError, message: "Invalid xain_callback in config. Use {Phoenix.HTML, :raw}"
  end

  @doc false
  def registered, do: Application.get_env(:phx_admin, :modules, []) |> Enum.reverse
  ...
  def default_resource_title_actions(%Plug.Conn{params: params} = conn, %{resource_model: resource_model} = defn) do
    singular = PhxAdmin.Utils.displayable_name_singular(conn) |> titleize
  ...
  def action_button(conn, defn, name, _page, action, actions, id \\ nil) do
    if action in actions do
      if PhxAdmin.Utils.authorized_action?(conn, action, defn) do
  ...
  def has_action?(conn, defn, action) do
    if PhxAdmin.Utils.authorized_action?(conn, action, defn),
