defmodule PhxAdmin.Controller do
  @moduledoc false
  ...
  def get_registered_by_controller_route(resource_name) do
    Enum.find PhxAdmin.get_registered, %{}, &(Map.get(&1, :controller_route) == resource_name)
  end

  defmacro __using__(_opts) do
    quote do
      import PhxAdmin.Controller

      def set_theme(conn, _) do
        assign(conn, :theme, PhxAdmin.theme)
      end

      def set_layout(conn, _) do
        layout = Application.get_env(:phx_admin, :layout) || "#{conn.assigns.theme.name}.html"
