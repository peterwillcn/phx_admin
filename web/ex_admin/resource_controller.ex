defmodule PhxAdmin.ResourceController do
  @moduledoc false

  defmacro __using__(_opts) do
    quote do
      alias PhxAdmin.Authorization
      import PhxAdmin.Utils
      import PhxAdmin.ParamsToAtoms
      import PhxAdmin.ParamsAssociations
      ...
      defp do_form_view(conn, resource, params) do
        ...
        if function_exported? model, :form_view, 3 do
          ...
          PhxAdmin.Form.default_form_view conn, resource, params
        end
      end

      defp handle_changeset_error(conn, defn, changeset, params) do
        conn = put_flash(conn, :inline_error, PhxAdmin.ErrorsHelper.create_errors(changeset, defn.resource_model))
        |> Plug.Conn.assign(:changeset, changeset)
        |> Plug.Conn.assign(:ea_required, changeset.required)
        contents = do_form_view(conn, PhxAdmin.Changeset.get_data(changeset), params)
      ...
      defp render_403(conn) do
        ...
        |> put_layout(false)
        |> put_status(403)
        |> render(PhxAdmin.ErrorView, "403.html")
        |> halt
      end
      ...
      def authorize_action(conn, action) do
        if PhxAdmin.Authorization.authorize_action(conn.assigns[:resource], conn, action) do
          conn
        else
          render_403(conn)
        end
      ...
      defp load_resource(conn, action, defn, resource_id) do
        ...
        |> PhxAdmin.Query.execute_query(repo(), action, resource_id)
      ...
      defp _handle_after_filter({conn, params, resource}, action, defn, [{name, opts} | t]) do
        ...
        if filter do
          case apply(defn.__struct__, name, [conn, params, resource, action]) do
            ...
            error ->
              raise PhxAdmin.RuntimeError, message: (gettext "invalid after_filter return:") <> " #{inspect error}"
          end
        else
      ...
      defp handle_plugs(conn, _action, defn) do
        case Application.get_env(:phx_admin, :plug, []) do
      ...
      defp repo, do: Application.get_env(:phx_admin, :repo)
    end
  end
end
