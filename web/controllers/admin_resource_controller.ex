defmodule PhxAdmin.AdminResourceController do
  @moduledoc false
  @resource nil

  use PhxAdmin.Web, :resource_controller
  ...
  def index(conn, defn, params) do
    ...
    page = case conn.assigns[:page] do
      nil ->
        ...
        |> PhxAdmin.Query.execute_query(repo(), :index, id)
    ...
    contents = if function_exported? model, :index_view, 3 do
      ...
      PhxAdmin.Index.default_index_view(conn, page, scope_counts)
    end
  ...
  def show(conn, defn, params) do
    ...
    contents = if function_exported? model, :show_view, 2 do
      ...
      PhxAdmin.Show.default_show_view(conn, resource)
    end
  ...
  def create(conn, defn, params) do
    ...
    case PhxAdmin.Repo.insert(changeset) do
      {:error, changeset} ->
  ...
  def update(conn, defn, params) do
    ...
    changeset = defn.resource_model.changeset(resource, params[defn.resource_name])
    case PhxAdmin.Repo.update(changeset) do
      {:error, changeset} ->
        conn |> handle_changeset_error(defn, changeset, params)
      {:ok, resource} ->
  ...
  def toggle_attr(conn, defn, %{attr_name: attr_name, attr_value: attr_value}) do
    ...
    render conn, "toggle_attr.js", attr_name: attr_name, attr_value: Map.get(resource, attr_name_atom), id: PhxAdmin.Schema.get_id(resource)
  end
  ...
  def destroy(conn, defn, params) do
    ...
    PhxAdmin.Repo.delete(resource, params[defn.resource_name])
    page =
    ...
    pagination =
      ...
      |> PhxAdmin.Paginate.paginate(page_number, page.page_size, page.total_pages, page.total_entries, "#{model_name}")
  ...
  def batch_action(conn, defn, %{batch_action: "destroy"} = params) do
    ...
    type = case PhxAdmin.Schema.primary_key(resource_model) do
      nil -> :integer
  ...
  def csv(conn, defn, params) do
    ...
    |> PhxAdmin.Query.execute_query(repo(), :csv, id)
    |> case  do
      [] -> []
      resources ->
        if function_exported? model, :build_csv, 1 do
          ...
          PhxAdmin.CSV.build_csv(resources)
        end
    end
