defmodule PhxAdmin.AdminAssociationController do
  @moduledoc false
  ...
  def index(conn, _defn, resource, %{"association_name" => association_name} = params) do
    ...
    page = PhxAdmin.Model.potential_associations_query(resource, defn_assoc.__struct__, assoc_name, params["keywords"])
    |> repo().paginate(params)

    results = page.entries
    |> Enum.map(fn(r) -> %{id: PhxAdmin.Schema.get_id(r), pretty_name: PhxAdmin.Helpers.display_name(r)} end)
  ...
  def add(conn, defn, resource, %{"association_name" => association_name, "selected_ids" => selected_ids} = params) do
    ...
    |> redirect(to: PhxAdmin.Utils.admin_resource_path(resource, :show))
  end
