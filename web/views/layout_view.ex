defmodule PhxAdmin.LayoutView do
  @moduledoc false
  ...
  def site_title do
    case Application.get_env(:phx_admin, :title) do
      nil ->
  ...
  def logo_mini do
    default = "Ex<b>A</b>"
    Application.get_env(:phx_admin, :logo_mini, default)
    |> Phoenix.HTML.raw
  end

  def logo_full do
    default = "Ex<b>Admin</b>"
    Application.get_env(:phx_admin, :logo_full, default)
    |> Phoenix.HTML.raw
  end
  ...
  def footer_content do
    ...
    Application.get_env(:phx_admin, :footer, default)
    |> Phoenix.HTML.raw
  end
  ...
  def admin_static_path(conn, path) do
    static_path conn, Path.join(["/", "themes", PhxAdmin.theme.name, path])
  end

  def theme_selector do
    Application.get_env(:phx_admin, :theme_selector)
    |> Enum.with_index
    |> theme_selector
  end
  ...
  defp theme_selector(options) do
    current = Application.get_env(:phx_admin, :theme)
